...
onmessage = function(dictionary) {
	caverphone = [];
	dictionary.data.split(',').forEach(function(element, index, array){
		//caverphone.push(element);
		caverphone.push(new String(element).caverphone());
	});
	done(caverphone.toString());
};
